...
const logger = defineObserver({
  ...
  onToolSkip(event) {
    console.log(`[tool] ${event.name} skipped by ${event.source}: ${event.reason}`);
  },
  onStepRetry(event) {
    console.log(`\n[retry] Step retried (attempt ${event.retries}): ${event.reason}`);
  },
  onStepFinish(event) {
  ...
  onRunFinish(event) {
    ...
    if (event.usage.cacheReadTokens)
      console.log(
        `[run] Cache: ${event.usage.cacheReadTokens} read / ${event.usage.cacheWriteTokens ?? 0} write`,
      );
...
const agent = defineAgent({
  ...
  tools: [getWeather, calculate, readUrl],
